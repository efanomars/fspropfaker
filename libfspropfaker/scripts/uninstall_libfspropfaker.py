...
# File:   uninstall_libfspropfaker.py

# Removes all files installed by the install_libfspropfaker.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_libfspropfaker.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f             {}/include/fspropfaker".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f              {}/lib/libfspropfaker.so*".format(sSudo, sInstallDir), shell=True)
		subprocess.check_call("{} rm    -f              {}/lib/libfspropfaker.a".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f       {}/lib/pkgconfig/fspropfaker.pc".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f        {}/share/doc/libfspropfaker".format(sSudo, sInstallDir).split())

	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f libfspropfaker/build".format(sSudo).split())
